...
(function(){
  const STORAGE_KEY='wishlist-ut-v2-release-0901';
  const VERSION=2;
  ...
  function read(){
    try{
      ...
      return parsed&&parsed.version===VERSION?parsed:emptyState();
    }catch(error){
  ...
  function exportCSV(){
    ...
    link.href=url;
    link.download=`wishlist-ut-v2-${new Date().toISOString().slice(0,10)}.csv`;
