...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  function renderResult(){
    ...
    const exploration=task.manualEnd?`<section class="form-card"><h2>탐색·선택 기록</h2><p>탭한 상품이 자동 입력됩니다. 말로 선택했거나 다른 상품을 최종 선택했다면 수정해 주세요.</p><label class="field"><span>선택 상품명 또는 ID</span><input name="selectedProductName" value="${escapeText(run.selectedProductName||run.selectedProductId||'')}"></label>${select('selectionSource','최종 선택 상품의 출처',[['','선택해 주세요'],['home-area','홈 찜 영역'],['home-other','홈의 다른 영역'],['revisit-sheet','다시 살펴볼 상품 시트'],['wishlist-list','찜 목록'],['search','검색'],['other','기타'],['none','선택하지 않음']],run.selectionSource||'',true)}${taskId==='t1'?select('homeAreaSeen','홈 찜 영역: 2초 이상 노출 또는 영역 내 탭',[['true','해당'],['false','해당하지 않음']],String(!!run.homeAreaSeen)):select('revisitOpened','다시 살펴볼 상품 시트를 열었는가',[['true','열었음'],['false','열지 않음']],String(!!run.revisitOpened))}${text('areaRecallAnswer',taskId==='t1'?'홈에서 어떤 상품 영역을 보셨나요?':'찜 화면에서 어떤 영역을 보셨나요?',run.areaRecallAnswer)}${text('areaMeaningAnswer',taskId==='t1'?'그 영역은 어떤 상품을 모아 보여주는 곳이라고 생각했나요?':'맨 위에 있던 묶음은 어떤 상품을 모아 둔 곳이라고 생각했나요?',run.areaMeaningAnswer)}<p>영역을 보지 않았거나 시트를 열지 않은 경우: 지금까지의 답변을 먼저 기록한 뒤 보여주고 이해를 확인합니다.</p><button type="button" class="secondary" data-show-area>영역 제시 후 이해 확인</button>${select('shownAfter','영역을 제시한 뒤 이해를 확인했는가',[['false','아니요'],['true','예']],String(!!run.shownAfter))}${text('shownMeaningAnswer','제시 후 이해 답변',run.shownMeaningAnswer)}</section>`:'';
