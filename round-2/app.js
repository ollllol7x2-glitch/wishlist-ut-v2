...
function renderProducts(category='전체'){
  ...
  const activeProducts=wishlistProducts.filter(product=>!state.deletedProducts.has(product.id)&&(!state.cleanupIds.has(product.id)||state.restoredArchive.has(product.id)));
  const visible=category==='전체'?activeProducts:activeProducts.filter(product=>product.category===category);
  const cleanedTotal=428-state.cleanupCount-state.editDeletedCount+state.restoredArchive.size;
  productCount.textContent=`상품 ${category==='전체'?cleanedTotal:Math.max(0,totals[category]-wishlistProducts.filter(p=>p.category===category&&(state.deletedProducts.has(p.id)||state.cleanupIds.has(p.id))&&!state.restoredArchive.has(p.id)).length)}`;
  ...
  if(hiddenChip){
    ...
    hiddenChip.setAttribute('aria-label',`정리한 상품 ${hiddenCount}개 보기`);
    const label=hiddenChip.querySelector('span:last-child');
    if(label)label.textContent=`정리한 상품 ${hiddenCount}`;
  }
...
function v24SelectCard(p,index,{archive=false,selectable=true}={}){
  ...
  const archiveIndex=[...oldProducts.slice(0,5),extraProducts[0]].findIndex(item=>item.id===p.id);
  ...
  return `<button class="v24-select-card${selected?' is-selected':''}${selectable?'':' is-view-only'}" type="button" data-action="${selectable?(archive?'toggle-archive':'toggle-edit'):'open-detail'}" data-product-id="${p.id}">
    <span><img src="assets/products/${p.image}" alt="${p.name}">${selectable?`<i>${selected?'✓':''}</i>`:''}${archive?`<em>${folders[Math.max(0,archiveIndex)%3]}</em>`:''}</span>
...
function homeTemplate(){
  ...
  return `<div class="screen-view v24-home">
    ...
    <section class="v24-existing"><header><h2>취향에 맞춘 상품</h2><small>Sponsored</small></header><div class="v24-home-feed-filters"><b>전체</b><span>주말직진</span><span>뷰티위크</span><span>스커트</span></div><div>${wishlistBaseProducts.slice(0,9).map(p=>v24MiniCard(p)).join('')}</div></section>
  </div>`;
}
...
function archiveTemplate(){
  ...
  const banner=hasCandidates?`<button class="v25-hidden-banner" data-action="open-cleanup" data-origin="archive-banner"><span class="hidden-eye-icon" aria-hidden="true"></span><span><b>오래 담아둔 상품이 28개 있어요</b><small>한 번에 정리하고 필요할 때 다시 꺼내보세요</small></span><strong>정리하기</strong></button>`:'';
  const resultNote=completed?`<div class="v25-hidden-result"><b>28개 상품을 정리했어요.</b><span>찜 ${428-state.cleanupCount-state.editDeletedCount+state.restoredArchive.size}개 · 정리한 상품 ${count}개</span><small>30일 안에 원래 폴더로 되돌릴 수 있어요.</small></div>`:'';
  const empty=`<section class="v25-hidden-empty"><span class="hidden-eye-icon is-large" aria-hidden="true"></span><h2>${hasCandidates?'아직 정리한 상품이 없어요':'정리한 상품이 없어요'}</h2><p>${hasCandidates?'오래 담아둔 상품 28개를 한 번에 숨기고 30일 안에 되돌릴 수 있어요.':'상품을 숨기면 30일 동안 여기서 확인하고 원래 폴더로 되돌릴 수 있어요.'}</p></section>`;
  const selectedCount=state.selectedArchive.size;
  return `<div class="screen-view v24-archive v25-hidden-screen"><header class="v24-centered-head"><button data-action="go-back" aria-label="뒤로">‹</button><h1>정리한 상품 <button data-action="archive-info" aria-label="정리한 상품 안내">i</button></h1>${headerAction||'<span></span>'}</header>${banner}${resultNote}<div class="v24-archive-tools"><span>정리한 상품 ${count}개${editing?` · ${selectedCount}개 선택`:''}</span>${editing?'<button data-action="select-all-archive">전체선택</button>':isEmpty?'':'<button>최신순 ▾</button>'}</div>${isEmpty?empty:`<div class="v24-select-grid">${archiveItems.map((p,i)=>v24SelectCard(p,i,{archive:true,selectable:editing})).join('')}</div>${!editing&&!completed?'<p class="v25-hidden-note">정리한 상품은 찜 검색에서 제외되며 원래 폴더 정보는 유지돼요.</p>':''}`}${editing?`<div class="v24-archive-actions"><button data-action="delete-archive" ${selectedCount?'':'disabled'}>완전 삭제</button><button data-action="restore-selected" ${selectedCount?'':'disabled'}>목록으로 되돌리기</button></div>`:''}</div>`;
}

function cleanupBackdropTemplate(){
  const cards=wishlistProducts.slice(0,6);
  return `<div class="v24-cleanup-backdrop"><header class="v24-centered-head"><span></span><h1>찜</h1><span></span></header><div class="v24-select-grid">${cards.map((p,i)=>v24SelectCard(p,i,{selectable:false})).join('')}</div></div><div class="v24-cleanup-scrim"></div>`;
}
function cleanupTemplate(){
  const result=state.cleanupResult;
  const inner=!result?`<h2>오래 담아둔 28개를<br>한 번에 정리할까요?</h2><small>30일 이상 담아둔 상품 · 모든 폴더 기준</small><div class="v24-cleanup-chips"><span>기본 폴더 12개</span><span>겨울옷 9개</span><span>스타일링 7개</span></div><p>정리한 상품에서 30일 안에 되돌릴 수 있어요.<br>30일이 지나면 자동으로 삭제돼요.</p><div class="v24-cleanup-buttons"><button data-action="cleanup-choice" data-choice="keep">다음에</button><button data-action="cleanup-choice" data-choice="clean">정리하기</button></div>`:`<i class="v24-result-check is-dark">✓</i><h2>28개 상품을 정리했어요</h2><p>정리한 상품에서 30일 안에 되돌릴 수 있어요.<br><small>찜 400개 · 정리한 상품 42개</small></p><div class="v24-cleanup-buttons"><button data-route="wishlist">확인</button><button data-action="open-archive-after-cleanup">정리한 상품 보기</button></div>`;
...
function editTemplate(){
  const editItems=wishlistProducts.filter(p=>!state.deletedProducts.has(p.id)&&(!state.cleanupIds.has(p.id)||state.restoredArchive.has(p.id)));
  const count=state.selectedEdit.size;
  return `<div class="screen-view v24-edit"><header class="v24-centered-head"><span></span><h1>상품 선택</h1><button class="v24-pink-done" data-route="wishlist">완료</button></header>${state.cleanupApplied?'':`<button class="round2-edit-banner" data-action="open-cleanup" data-origin="edit-banner"><span>오래 담아둔 28개를 한 번에 정리할까요?</span><b>정리하기 ›</b></button>`}<div class="v24-edit-tools"><span>기본 폴더 › 전체 (${428-state.cleanupCount-state.editDeletedCount+state.restoredArchive.size}) · ${count}개 선택</span><button data-action="select-all">전체선택</button></div><button class="round2-archive-entry" data-route="archive">정리한 상품 보기 ›</button><div class="v24-select-grid">${editItems.map((p,i)=>v24SelectCard(p,i)).join('')}</div><div class="v24-edit-actions"><button data-action="edit-to-top"><i>⇧</i>맨위로</button><button data-action="move-selected" ${count?'':'disabled'}><i>□</i>폴더이동</button><button data-action="delete-edit" ${count?'':'disabled'}><i>×</i>삭제${count?` (${count})`:''}</button></div></div>`;
}
function reportUTEvent(type,patch={}){
  if(!utMode||window.parent===window)return;
  window.parent.postMessage({type:'wishlist-ut-observation',task:utTask,participant:utParticipant,condition:utCondition,eventType:type,...patch},location.origin);
}
function completeEditDeletion(count){
  if(utMode&&utTask==='t3'&&count>=20)window.parent.postMessage({type:'wishlist-ut-complete',task:utTask,participant:utParticipant,condition:utCondition,itemCount:count,cleanupPath:'edit-delete',proposalUsed:false,screen:'edit-deleted',route:reviewLabel.textContent},location.origin);
}
...
function updateReviewLabel(label){
  const labels={wishlist:'A1 · 찜 기본 폴더',home:'C1 · 홈 재방문 유도',search:'A3/A3b/A4 · 찜 내 검색',price:'B2 · 가격 하락 + 맥락 추천',notifications:'C2 · 알림 통합 피드',archive:`${state.archiveVariant} · 정리한 상품`,cleanup:'G3a/G3b/G3c · 오래 담아둔 상품 정리',edit:'E0 · 상품 선택',cart:'F2 · 빈 장바구니',lockscreen:'G1p · 재입고 푸시',detail:`상품 상세 · ${getProduct(state.productId).name}`};
...
function reportUTCompletion(){
  ...
  if(utTask==='t3'&&state.route==='cleanup'&&state.cleanupResult==='clean'){
    window.parent.postMessage({
      ...
      task:utTask,
      condition:utCondition,
      itemCount:28,cleanupPath:state.cleanupOrigin,proposalUsed:true,
      route:reviewLabel.textContent,
...
document.addEventListener('click',event=>{
  ...
  if(target.dataset.route){
    ...
    if(target.dataset.route==='cleanup'){state.cleanupResult=null;state.cleanupOrigin='archive-banner';}
    if(target.dataset.route==='archive')reportUTEvent('archive-entry',{archiveEntryFound:true});
  ...
  if(action==='open-detail'){
    const source=target.closest('.v24-home-revisit')?'home-area':target.closest('.revisit-list')?'revisit-sheet':state.route==='search'?'search':state.route==='wishlist'?'wishlist-list':state.route==='home'?'home-other':state.route;
    reportUTEvent('product-choice',{selectedProductId:target.dataset.productId,selectedProductName:getProduct(target.dataset.productId).name,selectionSource:source,homeAreaSeen:source==='home-area'?true:undefined});
    closeOverlay();state.utCandidate=target.dataset.utCandidate||'';showRoute('detail',{productId:target.dataset.productId});
  }
  if(action==='open-revisit'){reportUTEvent('revisit-open',{revisitOpened:true});showRevisit();}
  ...
  if(action==='open-cleanup'){closeOverlay();state.cleanupOrigin=target.dataset.origin||'archive-banner';state.cleanupResult=null;showRoute('cleanup');}
  if(action==='cleanup-choice'){
    if(target.dataset.choice==='keep'){reportUTEvent('proposal-dismiss');state.cleanupResult=null;showRoute('wishlist');}
    else{state.cleanupApplied=true;state.cleanupCount=28;state.cleanupIds=new Set(cleanupCandidates.map(p=>p.id));state.cleanupResult='clean';renderProducts();renderRoute();showToast('정리한 상품에서 30일 안에 되돌릴 수 있어요');window.setTimeout(reportUTCompletion,60);}
  }
  ...
  if(action==='toggle-edit'){
    const id=target.dataset.productId;state.selectedEdit.has(id)?state.selectedEdit.delete(id):state.selectedEdit.add(id);
    target.classList.toggle('is-selected',state.selectedEdit.has(id));target.querySelector('.v24-select-card > span > i').textContent=state.selectedEdit.has(id)?'✓':'';
    const count=state.selectedEdit.size;routeScreen.querySelector('.v24-edit-tools > span').textContent=`기본 폴더 › 전체 (${428-state.cleanupCount-state.editDeletedCount+state.restoredArchive.size}) · ${count}개 선택`;
    const button=routeScreen.querySelector('[data-action="delete-edit"]');button.disabled=!count;button.innerHTML=`<i>×</i>삭제${count?` (${count})`:''}`;
    routeScreen.querySelector('[data-action="move-selected"]').disabled=!count;
  }
  if(action==='delete-edit'&&state.selectedEdit.size){const count=state.selectedEdit.size;state.selectedEdit.forEach(id=>state.deletedProducts.add(id));state.editDeletedCount+=count;state.selectedEdit.clear();renderProducts();showRoute('wishlist');showToast(`${count}개 상품을 삭제했어요`);completeEditDeletion(state.editDeletedCount);}
  if(action==='edit-to-top')content.scrollTo({top:0,behavior:'smooth'});
  ...
  if(action==='restore-selected'&&state.selectedArchive.size){
    ...
    if(restoredTarget)window.setTimeout(reportUTCompletion,60);
  }
...
function openHashState(hash){
  ...
  if(key==='A1'){showRoute('wishlist');if(utMode&&utTask==='t3'&&!state.cleanupApplied){state.cleanupOrigin='automatic-sheet';showRoute('cleanup');}return;}
  if(key==='A1C'){state.cleanupCount=28;state.cleanupIds=new Set(cleanupCandidates.map(p=>p.id));state.cleanupApplied=true;state.cleanupResult='clean';state.archiveVariant='E3e';renderProducts();showRoute('wishlist');return;}
